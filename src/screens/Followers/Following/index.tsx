...
const Following = ({userID}: Props) => {
  ...
  useEffect(() => {
    const FetchFollowing = async () => {
      try {
        ...
        const url = `https://api.neynar.com/v2/farcaster/following?fid=${ID}&limit=100`;
        const response = await axios.get(url, options);

        const followingsData = response.data.users.map(user => {
          return {
            id: user?.user.fid,
  ...
  const handleFollowing = async (id: string) => {
    try {
      ...
      console.log(unFollow.status, 'unfollow****');
